...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES["gambar_barang"]["tmp_name"], $target_file)) {
        ...
        $stmt = $koneksi->prepare("UPDATE barang SET nama_barang=?, jenis_barang=?, harga_barang=?, gambar_barang=? WHERE id=?");
        $stmt->bind_param("sssss", $nama_barang, $jenis_barang, $harga_barang, basename($_FILES["gambar_barang"]["name"]), $id);
        $stmt->execute();
